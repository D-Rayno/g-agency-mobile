// components/form/layout.tsx
/**
 * Form Layout Component - Maximum Stability Wrapper
 * Pure static values - no theme hooks
 */
...
const UltraStableContentWrapper = memo(
...
function StableFormLayout<T extends FieldValues>({
  ...
  // Create ultra-stable form instance
  const stableFormConfig = useMemo(() => {
    ...
    formConfig.defaultValues
      ? JSON.stringify(formConfig.defaultValues)
      : undefined,
    debugLog,
  ]);
  ...
  if (enableKeyboardAvoidance) {
    content = <KeyboardAvoidingView {...stableKeyboardProps}>{content}</KeyboardAvoidingView>;
  }
...
export const FormLayout = memo(StableFormLayout, (prevProps, nextProps) => {
  const configChanged =
    ...
    JSON.stringify(prevProps.formConfig.defaultValues) !==
      JSON.stringify(nextProps.formConfig.defaultValues);
